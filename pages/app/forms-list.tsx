...
export const getAllForms = async (token: string): Promise<HHForm[]> => {
  const response = await fetch(`http://dotw-hikma.azurewebsites.net/admin_api/get_event_forms`, {
    method: 'GET',
    headers: {
      Authorization: token,
    },
...
const deleteForm = async (id: string, token: string): Promise<any> => {
  const response = await fetch(`http://dotw-hikma.azurewebsites.net/admin_api/delete_event_form`, {
    method: 'DELETE',
    headers: {
      Authorization: token,
...
export default function FormsList() {
  ...
  */
  ...
  const toggleFormField = (id: string, field: string) => (event: any) => {
    ...
    const url = `http://dotw-hikma.azurewebsites.net/admin_api/${endpoint}`;
    axios
      .post(
        url,
        {
